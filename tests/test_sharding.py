...
def test_two_shards_partition_all_projects():
    ...
    shard_1 = shard_projects(projects, shard=1, num_shards=2, ecosystem_projects={})
...
def test_three_shards_partition_all_projects():
    ...
    for s in range(3):
        sharded = shard_projects(projects, shard=s, num_shards=3, ecosystem_projects={})
...
def test_bin_packing_spreads_expensive_projects():
    ...
    s0 = shard_projects(projects, shard=0, num_shards=2, ecosystem_projects=ep)
    s1 = shard_projects(projects, shard=1, num_shards=2, ecosystem_projects=ep)
...
def test_more_shards_than_projects():
    ...
    all_sharded: list[str] = []
    for s in range(5):
        shard_result = shard_projects(
            projects, shard=s, num_shards=5, ecosystem_projects={}
        )
